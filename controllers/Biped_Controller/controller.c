...
void robot_control()
{
    ...
    st_f.data[0][0] = (Ty + st_f.data[1][0] * robot.toe[robot.st].x) / robot.toe[robot.st].y;
    //求支撑足关节力
...
void forwardKinematics()
{
    ...
    for (legNameTypeDef legIdx = L; legIdx <= R; legIdx++)
    {
        ...
        robot.toe[legIdx].y = -a0 * c0 - a1 * c01;
    }
...
void update_toe0_dtoe0()
{
    ...
    else
    {
        ...
        st_dtoe.y = (robot.toe[robot.st].y - pre_st_toe.y) / (0.001 * ((double)TIME_STEP));
    }
...
void curve(vect2TypeDef *ft, vect2TypeDef *dft, double xT, vect2TypeDef toe0, vect2TypeDef dtoe0)
{
    ...
    if (t < Tf / 2.0)
    {
        ft->y = 16 * (z0 - zh) * t * t * t / (Tf * Tf * Tf) + 12 * (zh - z0) * t * t / (Tf * Tf) + z0;
        dft->y = 16 * (z0 - zh) * 3 * t * t / (Tf * Tf * Tf) + 12 * (zh - z0) * 2 * t / (Tf * Tf);
    }
    else
    {
        ft->y = 4 * (z0 - zh) * t * t / (Tf * Tf) - 4 * (z0 - zh) * t / Tf + z0;
